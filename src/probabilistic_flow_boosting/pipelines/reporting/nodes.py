...
from probabilistic_flow_boosting.models.resflowEmb import ResFlow as ResFlowEmb
from probabilistic_flow_boosting.models.resflowGauss import ResFlowGauss, ResFlowGaussDataModule
from probabilistic_flow_boosting.models.resflow_cnf import ResFlowCNF
...
def calculate_entropy(data, bins=100):
        # Create histogram
        ...
        # plt.savefig(f'entropy_hist_{entropy}.png')
        # pdb.set_trace()
...
def calculate_metrics_nodeflow(
    ...
    entropy = np.array([calculate_entropy(samples[i, :]) for i in range(samples.shape[0])])
    peak_std = np.array([(np.sum(np.sqrt((peaks_1[i]-samples[i,:])**2))/1000) for i in range(samples.shape[0])])
    invstd_score_entropy = 1/entropy
    invstd_score_peak_std = 1/peak_std

    y_test_df = pd.DataFrame(list(zip(*[y_test.squeeze(), peaks_1.squeeze(), peaks_2[:,1].squeeze(), entropy, peak_std, invstd_score_entropy, invstd_score_peak_std])), columns = ["true", "peak1", "peak2", "entropy", "peak_std", "invstd_score_entropy", "invstd_score_peak_std"])
    error_dict ={}
    percentile_values = {}
    for col in ["invstd_score_entropy", "invstd_score_peak_std"]:
        error_dict[col] = {}
        error_dict[col]['rmse_1'] = {}
        error_dict[col]['mae_1'] = {}
        y_test_df = y_test_df.sort_values(by=col) # sort by score
        percentiles = np.arange(0,100,10) # percentiles
        percentile_values[col] = np.percentile(y_test_df[col], percentiles)
        for i, percentile in enumerate(percentiles):
            # Get the subset of the original DataFrame for the current percentile
            # pdb.set_trace()
            subset = y_test_df[y_test_df[col] >= percentile_values[col][i]]
            rmse_1 = np.sqrt(np.mean((subset["true"]-subset["peak1"])**2))
            mae_1 = mean_absolute_error(subset["true"],subset["peak1"])
            # add to dict
            error_dict[col]['rmse_1'][percentile] = rmse_1
            error_dict[col]['mae_1'][percentile] = mae_1

    
    mean_entropy_rmse1 = float(np.mean([val for val in error_dict["invstd_score_entropy"]["rmse_1"].values()]))
    mean_entropy_mae1 = float(np.mean([val for val in error_dict["invstd_score_entropy"]["mae_1"].values()]))
    mean_peak_std_rmse1 = float(np.mean([val for val in error_dict["invstd_score_peak_std"]["rmse_1"].values()]))
    mean_peak_std_mae1 = float(np.mean([val for val in error_dict["invstd_score_peak_std"]["mae_1"].values()]))
    uncertainty_time = time.time() - start_uncertainty
    error_df = pd.DataFrame(np.array([[perc for perc in percentile_values["invstd_score_entropy"]], [perc for perc in percentile_values["invstd_score_peak_std"]], [val for val in error_dict["invstd_score_entropy"]["rmse_1"].values()], [val for val in error_dict["invstd_score_peak_std"]["rmse_1"].values()], [val for val in error_dict["invstd_score_entropy"]["mae_1"].values()], [val for val in error_dict["invstd_score_peak_std"]["mae_1"].values()]]).transpose(), columns=["percentile_entropy", "percentile_peakstd",  "invstd_score_entropy_rmse", "invstd_score_peak_std_rmse", "invstd_score_entropy_mae", "invstd_score_peak_std_mae"])
    ...
    # return nll, rmse_1, rmse_2, crpss.mean(), nll_time, rmse_time, crps_time # mean_entropy_rmse1, mean_entropy_mae1, mean_peak_std_rmse1, mean_peak_std_mae1,  uncertainty_time, error_df
    return nll, rmse_1, rmse_2, crpss.mean(), mean_entropy_rmse1, mean_entropy_mae1, mean_peak_std_rmse1, mean_peak_std_mae1, nll_time, rmse_time, crps_time, uncertainty_time, error_df

def calculate_metrics_resflow_cnf(
        model: ResFlowCNF,
        X_train: np.ndarray,
        y_train: np.ndarray,
        X_test: np.ndarray,
    ...
    # NLL
    # pdb.set_trace()
    datamodule = ResFlowDataModule(X_train, y_train, X_test, y_test, split_size=0.8, batch_size=sample_batch_size)
    trainer = Trainer(
        max_epochs=100,
        devices=1,
        check_val_every_n_epoch=4,
    ...
    # entropy = np.array([calculate_entropy(samples[i, :]) for i in range(samples.shape[0])])
    # peak_std = np.array([(np.sum(np.sqrt((peaks_1[i]-samples[i,:])**2))/1000) for i in range(samples.shape[0])])
    # print(f"entropy : {entropy}, peak_std : {peak_std}")
    # invstd_score_entropy = 1/entropy
    # invstd_score_peak_std = 1/peak_std
...
def calculate_metrics_resflowGauss(
        model: ResFlowGauss,
        X_train: np.ndarray,
        y_train: np.ndarray,
        X_test: np.ndarray,
    ...
    # NLL
    # pdb.set_trace()
    datamodule = ResFlowGaussDataModule(X_train, y_train, X_test, y_test, split_size=0.8, batch_size=batch_size)
    datamodule.setup("test") # setup to scale the dataset
    trainer = Trainer(
        max_epochs=100,
        devices=1,
        check_val_every_n_epoch=4,
    ...
    from probabilistic_flow_boosting.models.resflowGauss.augmentations import embed_data_mask_mlp, embed_data_mask_mlp_cont
    nll=0
    model.eval()
    ys = []
    pred_ys=[]
    ...
    for i, (x, y) in enumerate(datamodule.test_dataloader()):
        x_cont = torch.tensor(x).to(torch.float32).to("cuda")
        con_mask = torch.ones_like(x_cont).long()
        x_cont_enc = embed_data_mask_mlp_cont(x_cont, con_mask, model.base_model) 
        y_pred = model.base_model(x_cont_enc)
        y_pred_cpu = y_pred.detach().cpu() 
        # print("Gaussian NLL:", loss_fn(y_pred_cpu[:,0].unsqueeze(dim=-1), y, y_pred_cpu[:,1].unsqueeze(dim=-1)))
        # pdb.set_trace()
        y_pred_cpu[:, 1] = np.log(np.sqrt(y_pred_cpu[:, 1]))  # Transform var to log std / CatBoost RMSEWithUncertainty

        distribution = ConditionalDiagonalNormal(shape=[1])  # Assume 1D distribution
        nll += -distribution.log_prob(y, y_pred_cpu).numpy().mean()
        mean, std = distribution._compute_params(y_pred_cpu)
        ys.append(y.detach().cpu().squeeze()) # save the predictions
        pred_ys.append(y_pred_cpu[:,0])
        pred_stds.append(y_pred_cpu[:,1])
        # print(i, nll)
    # pdb.set_trace()
    ys = np.concatenate(ys)
    pred_ys = np.concatenate(pred_ys)
    pred_stds = np.concatenate(pred_stds)
    final_nll = nll/(i+1)
    print("final_nll :", final_nll)
    nll_time = time.time() - start_nll

    start_rmse = time.time()
    # RMSE
    rmse_1 = np.sqrt(np.mean((ys-pred_ys)**2))
    rmse_2 = np.sqrt(np.mean((ys-pred_ys)**2))
    rmse_time = time.time() - start_rmse
    ...
    # print(f"entropy : {entropy}, peak_std : {peak_std}")
    # invstd_score_entropy = 1/entropy
    invstd_score_peak_std = 1/pred_stds

    y_test_df = pd.DataFrame(list(zip(*[ys, pred_ys, pred_ys, pred_stds, invstd_score_peak_std])), columns = ["true", "peak1", "peak2", "peak_std", "invstd_score_peak_std"])
    error_dict ={}
    percentile_values = {}
    for col in ["invstd_score_peak_std"]:
        error_dict[col] = {}
        error_dict[col]['rmse_1'] = {}
        error_dict[col]['mae_1'] = {}
        y_test_df = y_test_df.sort_values(by=col) # sort by score
    ...
    mean_peak_std_mae1 = float(np.mean([val for val in error_dict["invstd_score_peak_std"]["mae_1"].values()]))
    uncertainty_time = time.time() - start_uncertainty
    error_df = pd.DataFrame(np.array([[perc for perc in percentile_values["invstd_score_peak_std"]], [val for val in error_dict["invstd_score_peak_std"]["rmse_1"].values()], [val for val in error_dict["invstd_score_peak_std"]["mae_1"].values()]]).transpose(), columns=[ "percentile_peakstd", "invstd_score_peak_std_rmse", "invstd_score_peak_std_mae"])
    pprint.pprint(error_dict)

    # CRPS
    start_crps = time.time()
    crpss = ps.crps_gaussian(
        ys,
        pred_ys,
        np.exp(pred_stds)
    ).mean()
    crps_time = time.time() - start_crps
    # pdb.set_trace()
    # return final_nll, rmse_1, rmse_2, crpss, nll_time, rmse_time, crps_time # mean_entropy_rmse1, mean_entropy_mae1, mean_peak_std_rmse1, mean_peak_std_mae1,  uncertainty_time, error_df
    return final_nll, rmse_1, rmse_2, crpss, mean_peak_std_rmse1, mean_peak_std_mae1, nll_time, rmse_time, crps_time, uncertainty_time, error_df



def calculate_metrics_resflowEmb(
        model: ResFlowEmb,
        X_train: np.ndarray,
        y_train: np.ndarray,
        X_test: np.ndarray,
...
def plot_metrics_resflow(
    ...
    samples[-1] = np.atleast_2d(samples[-1])
    samples = np.concatenate(samples).reshape(-1, 1000)
    os.makedirs(f"output/results/ablation/{dataset}", exist_ok=True)
    n=5
    fig, axs = plt.subplots(1, n, figsize=(20, 4))
    rand_ints = np.random.randint(1, y_test.shape[0], n)
    for i, ax in enumerate(axs):
        # Create the histogram
        sns.histplot(samples[rand_ints[i],:], kde=True,  bins=100, color="skyblue", label="Distribution", ax=ax)

        # Add the true value as a vertical line
        ax.axvline(x=y_test[rand_ints[i]], color="red", linestyle="--", linewidth=3, label="True Value")

        # # Customize the plot
        # plt.xlabel("Value",fontsize=20)
        # plt.ylabel("Frequency", fontsize=20)
        # plt.title("KDE density plot with True Value")
        ax.legend()
    plt.tight_layout()
    plt.savefig(f"output/results/ablation/{dataset}/{dataset}_{fold}_{str(rand_ints).replace(' ', '_')}.png")
