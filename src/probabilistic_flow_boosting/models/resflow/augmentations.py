import torch
import numpy as np
from probabilistic_flow_boosting.models.resflow.num_embedding import compute_bins, PiecewiseLinearEncoding, PiecewiseLinearEmbeddings
import pdb

def embed_data_mask(x_categ, x_cont, cat_mask, con_mask, model, vision_dset=False):
    device = x_cont.device
    x_categ = x_categ + model.categories_offset.type_as(x_categ) # pad for special tokens
    x_categ_enc = model.embeds(x_categ) # embed categorical data
    n1, n2 = x_cont.shape
    _, n3 = x_categ.shape
    if model.cont_embeddings == 'MLP':
        x_cont_enc = torch.empty(n1, n2, model.dim) # pass each numerical column through MLP layers and generate embeddings.
...
def embed_data_mask_mlp(x_categ, x_cont, cat_mask, con_mask, model, vision_dset=False):
    device = x_cont.device
    x_categ = x_categ + model.categories_offset.type_as(x_categ) # pad for special tokens
    x_categ_enc = model.embeds(x_categ) # embed categorical data torch.Size([64, 37, 8])
    n1, n2 = x_cont.shape
    _, n3 = x_categ.shape
    ...
    ## add aditional embedding options here.
    elif model.cont_embeddings == 'None': 
        # for i in range(model.num_continuous):
        x_cont_enc = x_cont.unsqueeze(2).repeat(1, 1, model.dim) # no encoding just repeat the numerical embedding to match the size.

    ## only embed categorical options here.
    elif model.cont_embeddings == 'PLE': 
        # pdb.set_trace()
        # for i in range(model.num_continuous):
        # print("No encoding for Numerical data")
        x_cont_enc = x_cont.unsqueeze(2).repeat(1, 1, model.dim) # no encoding just repeat the numerical embedding to match the size.
...
def embed_data_mask_mlp_cont(x_cont, con_mask, model, vision_dset=False):
    device = x_cont.device
    # x_categ = x_categ + model.categories_offset.type_as(x_categ) # pad for special tokens
    # x_categ_enc = model.embeds(x_categ) # embed categorical data torch.Size([64, 37, 8])
    n1, n2 = x_cont.shape
